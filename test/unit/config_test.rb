...
class ConfigTest < ActiveSupport::TestCase
  ...
  test 'config instance should only have :glyphs, :param_name, :left_over, and :all as options' do
    assert_equal [:glyphs, :param_name, :left_over, :all], GlyphFilter.config.config.keys
  end
  {:glyphs => ("A".."Z").to_a, :param_name => :glyph, :left_over => "?", :all => "ALL"}.each_pair do |name, config|
